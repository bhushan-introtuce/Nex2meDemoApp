...
public class InvertedColorsVideoRenderer extends BaseVideoRenderer {
    ...
    private FrameListner frameListner;
    ...
    public void onFrame(Frame frame) {
        ...
        if(frameListner!=null){
            Log.d("FRAME_DEBUG","listner found");
            rgbFrame = frameToBitmap(frame);
            frameListner.onFrame(rgbFrame);
        }
        else {
            Log.d("FRAME_DEBUG","Listener null");
        }
    }
    ...
    public Bitmap frameToBitmap(Frame frame){
        try {
            ...
            Bitmap tbmp2 = Bitmap.createBitmap(frame.getWidth(), frame.getHeight(), Bitmap.Config.ARGB_8888);
            Mat picyv12 = new Mat(frame.getHeight() * 3 / 2, frame.getWidth(), CvType.CV_8UC1);  //(im_height*3/2,im_width), should be even no...

            byte[] arr=new byte[frame.getBuffer().capacity()];
            frame.getBuffer().get(arr,0,frame.getBuffer().capacity());
            picyv12.put(0, 0, arr); // buffer - byte array with i420 data
            Imgproc.cvtColor(picyv12,picyv12,Imgproc.COLOR_YUV2BGR_YV12);

            long endTimei = SystemClock.uptimeMillis();
            Log.d("i420_time", Long.toString(endTimei - startTimei)+" ,"+frame.getWidth()+","+frame.getHeight());
