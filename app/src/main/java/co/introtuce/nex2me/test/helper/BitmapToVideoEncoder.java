...
public class BitmapToVideoEncoder {
    ...
    public void startEncoding(int width, int height, File outputFile,boolean flag) {
        Log.d(TAG, "starting encoding top " + outputFile);
        Log.d("REC_TEST","starting encoding " + outputFile);
        //startEncoding(width,height,outputFile);
    }
    public void startEncoding(int width, int height, File outputFile) {
        Log.d(TAG, "starting encoding " + outputFile);
        Log.d("REC_TEST","starting encoding " + outputFile);
        mWidth = width;
        mHeight = height;
    ...
    private void encodeYUV420SP(byte[] yuv420sp, int[] argb, int width, int height) {
        ...
        for (int j = 0; j < height; j++) {
            for (int i = 0; i < width; i++) {
                ...
                if (j % 2 == 0 && index % 2 == 0) {
                    Log.d("ERROR_FUN","Test "+uvIndex+1);
                    Log.d("ERROR_FUN","Test "+yuv420sp.length);
                    Log.d("ERROR_FUN","Test "+yuv420sp[uvIndex+1]);
                    Log.d("ERROR_FUN","Test "+((byte) ((U < 0) ? 0 : ((U > 255) ? 255 : U))));
